...
#'         \code{posmean}: Posterior means from dash fit of each sample
...
dash <- function(comp_data,
  ...
  matrix_lik <- matrix(0, dim(comp_data)[1], dim(conc_mat)[1])
  ...
  for(n in 1:dim(comp_data)[1]){
    ...
    matrix_lik[n,1] <- exp(logfac(n) - sum(sapply(x, function(y) return(logfac(y)))) + sum(x*log(conc_mat[k,]/sum(conc_mat[k,]))))
  }
